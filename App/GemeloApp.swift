...
struct GemeloApp: App {
        ...
        .commands {
            CommandMenu("Chats") {
                Button {
                    ...
                    Label("New Local Chat (\(coordinator.newChatShortcutHint))", systemImage: "plus.bubble")
                }

                Button {
                    coordinator.switchToNextChat()
                } label: {
                    Label("Switch Local Chat (\(coordinator.switchChatShortcutHint))", systemImage: "arrow.triangle.2.circlepath")
                }
                .disabled(coordinator.totalChatCount <= 1)

                Button {
                    coordinator.closeCurrentChat()
                } label: {
                    Label("Close Local Chat (\(coordinator.closeChatShortcutHint))", systemImage: "xmark.bubble")
                }
            }
    ...
    init() {
        ...
        coordinator.installLocalShortcutMonitorIfNeeded()
    }
}
