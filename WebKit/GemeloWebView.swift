...
struct GemeloWebView: NSViewRepresentable {
    ...
    func updateNSView(_ container: WebViewContainer, context: Context) {
        container.updateWebView(webView, coordinator: context.coordinator)
    }
    ...
    class Coordinator: NSObject, WKNavigationDelegate, WKUIDelegate, WKDownloadDelegate {
        ...
        func webView(_ webView: WKWebView, runJavaScriptTextInputPanelWithPrompt prompt: String, defaultText: String?, initiatedByFrame frame: WKFrameInfo, completionHandler: @escaping (String?) -> Void) {
            ...
            let textField = NSTextField(frame: NSRect(x: 0, y: 0, width: GemeloWebView.Constants.textFieldWidth, height: GemeloWebView.Constants.textFieldHeight))
        ...
        func webView(_ webView: WKWebView, requestMediaCapturePermissionFor origin: WKSecurityOrigin, initiatedByFrame frame: WKFrameInfo, type: WKMediaCaptureType, decisionHandler: @escaping (WKPermissionDecision) -> Void) {
            decisionHandler(origin.host.contains(GemeloWebView.Constants.trustedHost) ? .grant : .prompt)
        }
        ...
        private func isExternalURL(_ url: URL) -> Bool {
            ...
            // Keep only allowed service domains inside the app
...
class WebViewContainer: NSView {
    ...
    private var coordinator: GemeloWebView.Coordinator
    private var windowObserver: NSObjectProtocol?

    init(webView: WKWebView, coordinator: GemeloWebView.Coordinator) {
    ...
    func updateWebView(_ newWebView: WKWebView, coordinator newCoordinator: GemeloWebView.Coordinator) {
    ...
    private func attachWebView() {
        ...
        addSubview(webView)
        if let window = self.window, window.isKeyWindow, window.firstResponder !== webView {
